...
// Trust proxy when running behind Heroku or other proxies
app.set('trust proxy', 1);

// Global Request Logger (only verbose in non-production)
...
try {
  ...
  console.warn('Helmet could not be applied:', e && e.message);
}
...
// Log request body for POST/PUT requests only in non-production
if (process.env.NODE_ENV !== 'production') {
  app.use((req, res, next) => {
